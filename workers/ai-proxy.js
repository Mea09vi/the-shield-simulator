...
const THAILLM_MODELS = new Set([
    ...
    // ถอด 'qwen3.6-35b-a3b' ออก (v15.6.3): reasoning MoE กิน ~5k token กับ CoT
    // ที่เรา strip ทิ้ง → คำตอบชนเพดาน 8192 แล้วถูกตัดกลางประโยค (stop_reason:length)
    // + ช้า ~41s (2/3 ของ client timeout 90s). มติผู้ใช้ 24 ก.ค. 69: ไม่คุ้ม เอาออก
]);
...
async function callThaiLLM(env, model, system, userMsg) {
    ...
    // ตาข่ายกันเหนียว: บางโมเดล *-think (เช่น Pathumma-ThaiLLM-qwen3-8b-think) อาจปล่อย reasoning
    // เป็น preamble ("Here's a thinking process:…") แล้วปิดด้วย </think> เดี่ยว ๆ โดยไม่มี <think> เปิด
    // — regex คู่ด้านบนจับไม่ได้. หลัง strip คู่แล้ว ถ้ายังเหลือ </think> แปลว่าเป็น tag ปิดกำพร้า
    // → ตัดทุกอย่างจนถึงและรวม </think> ตัวสุดท้ายทิ้ง (ใช้ delimiter ของโมเดลเอง ไม่เดาด้วยสัดส่วน
    // อักษรไทย). ปลอดภัยกับโมเดลอื่น: ไม่มี </think> → ไม่แตะต้อง.  (เดิมพบใน qwen3.6-35b-a3b ที่ถอดออกแล้ว v15.6.3)
